...
const STATS = [
  { value: "500+",  label: { en: "Active ISPs",            pt: "ISPs ativos",              fr: "FAIs actifs"               } },
  { value: "2M+",   label: { en: "Managed subscribers",    pt: "Assinantes gerenciados",   fr: "Abonnés gérés"             } },
  { value: "99.9%", label: { en: "Uptime SLA",             pt: "Uptime SLA",               fr: "SLA de disponibilité"      } },
  { value: "45%",   label: { en: "Less rework",            pt: "Menos retrabalho",         fr: "Moins de retravail"        } },
];
...
export default function TurboISPSitePage() {
  ...
  const MODULES = [
    { icon: Users,        title: t("module_subscribers_title"),    desc: t("module_subscribers_desc"),    accent: "#fca311", bg: "rgba(252,163,17,0.08)",  border: "rgba(252,163,17,0.18)"  },
    { icon: Network,      title: t("module_noc_title"),            desc: t("module_noc_desc"),            accent: "#1AABF0", bg: "rgba(26,171,240,0.07)",   border: "rgba(26,171,240,0.15)"  },
    { icon: Map,          title: t("module_infrastructure_title"), desc: t("module_infrastructure_desc"), accent: "#10b981", bg: "rgba(16,185,129,0.07)",   border: "rgba(16,185,129,0.15)"  },
    { icon: DollarSign,   title: t("module_finance_title"),        desc: t("module_finance_desc"),        accent: "#8b5cf6", bg: "rgba(139,92,246,0.07)",   border: "rgba(139,92,246,0.15)"  },
    { icon: Wrench,       title: t("module_services_title"),       desc: t("module_services_desc"),       accent: "#ef4444", bg: "rgba(239,68,68,0.07)",    border: "rgba(239,68,68,0.15)"   },
    { icon: TrendingUp,   title: t("module_sales_title"),          desc: t("module_sales_desc"),          accent: "#06b6d4", bg: "rgba(6,182,212,0.07)",    border: "rgba(6,182,212,0.15)"   },
    { icon: ShoppingCart, title: t("module_inventory_title"),      desc: t("module_inventory_desc"),      accent: "#f59e0b", bg: "rgba(245,158,11,0.07)",   border: "rgba(245,158,11,0.15)"  },
    { icon: BarChart3,    title: t("module_reports_title"),        desc: t("module_reports_desc"),        accent: "#ec4899", bg: "rgba(236,72,153,0.07)",   border: "rgba(236,72,153,0.15)"  },
    { icon: Radio,        title: t("module_playbooks_title"),      desc: t("module_playbooks_desc"),      accent: "#14b8a6", bg: "rgba(20,184,166,0.07)",   border: "rgba(20,184,166,0.15)"  },
  ];
  ...
  const HERO_BADGE = s({ en: "ISP Management Platform",  pt: "Plataforma de gestão para ISPs", fr: "Plateforme de gestion pour FAI" });
  const SEE_MODULES = s({ en: "See modules",             pt: "Ver módulos",                    fr: "Voir les modules"               });
  const NAV_LOGIN   = s({ en: "Login",                   pt: "Entrar",                         fr: "Connexion"                      });
  const NAV_DEMO    = s({ en: "Request demo",            pt: "Solicitar demo",                 fr: "Demander une démo"              });
  ...
  const METRICS_LABEL = s({ en: "Real results",                        pt: "Resultados reais",                       fr: "Résultats réels"                               });
  const METRICS_TITLE = s({ en: "Numbers your operation will feel",    pt: "Números que a sua operação vai sentir",  fr: "Des chiffres que votre opération ressentira"   });
  const METRICS = [
    { stat: "99",  label: s({ en: "KPIs tracked in real time",        pt: "KPIs acompanhados em tempo real",        fr: "KPIs suivis en temps réel"                     }) },
    { stat: "45%", label: s({ en: "less rework backbone-to-billing",  pt: "menos retrabalho backbone-to-billing",   fr: "moins de retravail backbone-to-billing"         }) },
  ];

  const CTA_TITLE = s({ en: "Ready to turbocharge your operation?",      pt: "Pronto para turbinar sua operação?",       fr: "Prêt à turbocharger votre opération ?"          });
  const CTA_DESC  = s({ en: "Talk to a TurboISP specialist and see how the platform fits your reality.", pt: "Fale com um especialista TurboISP e veja como a plataforma se encaixa na sua realidade.", fr: "Parlez à un spécialiste TurboISP et découvrez comment la plateforme s'adapte à votre réalité." });
  const CTA_BTN   = s({ en: "Request demo",                               pt: "Solicitar demo",                           fr: "Demander une démo"                              });
  const FOOTER_COPY  = s({ en: "All rights reserved.",                    pt: "Todos os direitos reservados.",            fr: "Tous droits réservés."                          });
      ...
      >
        <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {/* Language switcher */}
            <div className="hidden md:flex items-center rounded-lg overflow-hidden border" style={{ borderColor: "rgba(0,0,0,0.1)" }}>
        ...
        >
          {TRUST_BADGES.map((badge) => (
            <span key={badge} className="flex items-center gap-1.5">
              <CheckCircle className="w-3 h-3" style={{ color: "rgba(26,171,240,0.65)" }} />
              {badge}
            </span>
          ))}
        </div>
      ...
      <section className="relative z-10 max-w-5xl mx-auto px-6 pb-16">
        ...
        >
          {STATS.map(({ value, label }) => (
            <div key={value} className="py-7 px-6 text-center" style={{ backgroundColor: "#fff" }}>
              <p className="text-3xl font-black mb-1" style={{ color: "#1AABF0" }}>{value}</p>
              <p className="text-[11px]" style={{ color: "rgba(10,20,40,0.45)" }}>{label[lang]}</p>
            </div>
          ))}
        </div>
      </section>
          ...
          {VALUES.map(({ icon: Icon, title, desc }) => (
            ...
            >
              <div
                ...
                style={{ backgroundColor: "rgba(26,171,240,0.08)", borderColor: "rgba(26,171,240,0.2)" }}
              >
      ...
      <section className="relative z-10 max-w-6xl mx-auto px-6 py-12">
        ...
        >
          <div
            ...
            style={{ backgroundColor: "rgba(26,171,240,0.15)", border: "1px solid rgba(26,171,240,0.25)" }}
          >
