...
export default function SecurityPage() {
  ...
  return (
    <div className="p-6 max-w-lg">
      ...
      {message && (
        <p className={`mb-4 text-xs rounded-md px-3 py-2 ${message.ok ? 'text-[#0F7A6C] bg-[#E6F5F2] border border-[#B7E0D8]' : 'text-[#C23B2E] bg-[#FDECEA] border border-[#F3C4BE]'}`}>
          {message.text}
        </p>
      )}
      ...
      <div className="rounded-xl border border-border bg-card/50 p-5">
        <div className="flex items-center gap-3 mb-4">
          {mfaEnabled
            ? <ShieldCheck className="w-5 h-5 text-[#0F7A6C]" />
        ...
        {setup && !mfaEnabled && (
          <form onSubmit={enableMfa} className="space-y-4">
            ...
            <a
              ...
              className="text-xs text-[#8A5A00] underline break-all"
            >
