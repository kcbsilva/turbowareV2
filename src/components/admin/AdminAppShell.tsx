...
export function AdminAppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-[100svh] w-full bg-background text-foreground">
      ...
      <nav
        className={`fixed inset-y-0 left-0 z-40 flex h-full shrink-0 flex-col border-r border-border bg-background p-2 transition-all duration-300 ease-in-out lg:static ${
          open ? 'w-64' : 'w-16'
        } ${mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}
      >
        <div className="mb-4 border-b border-border pb-4">
          <Link href="/admin" className="flex items-center gap-3 rounded-xl p-2 hover:bg-white/5">
            <span className="relative grid size-10 shrink-0 place-content-center">
              <span className="absolute inset-0 rounded-xl bg-white/10 blur-sm" />
              <Image
                src={turbowareLogo}
                alt=""
            ...
            {open && (
              <div className="min-w-0">
                <span className="block truncate text-sm font-semibold text-foreground">Turboware</span>
        ...
        {open && me && (
          <div className="mb-12 flex items-center gap-3 rounded-xl bg-white/5 px-2 py-2">
            <div className="grid size-8 shrink-0 place-content-center rounded-full bg-neutral-800 text-xs font-bold text-foreground">
              {initials}
            </div>
            <div className="min-w-0">
              <p className="truncate text-xs font-medium text-foreground">{me.name || 'Admin'}</p>
              <p className="truncate text-[10px] text-muted-foreground">{me.email || me.role}</p>
            </div>
          </div>
        )}
        ...
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          className="absolute bottom-0 left-0 right-0 hidden border-t border-border hover:bg-white/5 lg:block"
        >
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-14 shrink-0 items-center justify-between gap-3 border-b border-border bg-background/80 px-4 backdrop-blur-md">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="hidden truncate text-[11px] text-muted-foreground sm:block">Turboware Admin</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Link
              href="/admin/tickets"
              className="relative rounded-xl border border-border bg-card p-2 text-muted-foreground transition hover:text-foreground"
              aria-label="Tickets"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-xl border border-border bg-card px-2.5 py-2 text-xs font-medium text-muted-foreground hover:bg-white/5"
            >
    ...
    >
      {active && (
        <motion.span
          ...
          className="absolute inset-0 rounded-xl bg-white/10 shadow-[inset_3px_0_0_#fff]"
      ...
      {!!notifs && open && (
        <span className="relative z-10 ml-auto mr-3 flex h-5 min-w-5 items-center justify-center rounded-full bg-foreground px-1 text-[10px] font-bold text-background">
          {notifs}
        </span>
      )}
    </Link>
